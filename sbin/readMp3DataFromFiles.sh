...
# check parameters
if [ "$#" -lt 1 ]; then
    dofail "USAGE: readMp3DataFromFiles.sh CONFIGPROFILE] \nFATAL: requires 1 parameters " 1
    exit 1
fi
...
echo "start - readMp3DataFromFiles configProfile='${CONFIGPROFILE}'"
cd ${MYMM}
...
echo "done - readMp3DataFromFiles configProfile='${CONFIGPROFILE}'"
